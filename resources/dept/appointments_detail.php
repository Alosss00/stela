...
$sql = "SELECT a.*, e.full_name as employee_name, e.employee_code, e.position, e.department, e.contractor_company, e.verified_by as admin_verified_by, e.competency_name, p.position_name, p.position_type, u1.full_name as created_by_name, u2.full_name as approved_by_name, u_admin.full_name as admin_name, u_admin.username as admin_username, ktt1.full_name as ktt1_name, ktt2.full_name as ktt2_name FROM appointments a JOIN employees e ON a.employee_id = e.id LEFT JOIN positions p ON a.position_id = p.id LEFT JOIN users u1 ON a.created_by = u1.id LEFT JOIN users u2 ON a.approved_by = u2.id LEFT JOIN users u_admin ON e.verified_by = u_admin.id LEFT JOIN users ktt1 ON a.ktt1_approved_by = ktt1.id LEFT JOIN users ktt2 ON a.ktt2_approved_by = ktt2.id WHERE a.id = ? AND e.department = ?";
$appointment = $db->query($sql, [$id, $department], 'is')->fetch_assoc();
