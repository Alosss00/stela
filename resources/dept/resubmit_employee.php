        ...
        if (empty($full_name) || empty($position) || empty($department) || empty($competency_type) || empty($ruang_lingkup) || empty($contractor_company)) {
            ...
            if (!$error) {
                ...
                if (in_array('competency_name', $available_columns)) {
                    $set_parts[] = "competency_name = ?"; $params[] = $competency_name; $types .= "s";
                }
                if (in_array('supervision_area', $available_columns)) {
                    $set_parts[] = "supervision_area = ?"; $params[] = $supervision_area; $types .= "s";
                }
                if (in_array('sub_competency', $available_columns)) {
                    $set_parts[] = "sub_competency = ?"; $params[] = $sub_competency; $types .= "s";
                }
                if (in_array('statement_file', $available_columns)) {
                    $set_parts[] = "statement_file = ?"; $params[] = $statement_file; $types .= "s";
                }
                if (in_array('resubmit_count', $available_columns)) {
                    $set_parts[] = "resubmit_count = COALESCE(resubmit_count, 0) + 1";
                }
                if (in_array('resubmit_date', $available_columns)) {
                    $set_parts[] = "resubmit_date = NOW()";
                }
                ...
                if ($db->query($sql, $params, $types)) {
                    ...
                    if (!empty($employee['appointment_id'])) {
                        ...
                        $db->update('appointments', $appointmentData, ['id' => $appointment_id]);
                    }
                    ...
                    foreach ($cert_ids as $key => $cert_id) {
                        ...
                        if ($existing_id > 0) {
                            ...
                            $updateWhere = ['id' => $existing_id, 'employee_id' => $employee_id];
                            if (!$db->update('employee_certifications', $updateData, $updateWhere)) {
                    ...
                    if ($existing_appointment) {
                        ...
                        $updateApptData = [];
                        if ($expiry_date) {
                            $updateApptData['expiry_date'] = $expiry_date;
                        } else {
                            $updateApptData['expiry_date'] = null;
                        }

                        $updateApptData['status'] = 'draft';
                        
                        if ($is_ktt_resubmit) {
                            if ($existing_appointment['requires_ktt_msm_review'] == 1) {
                                $updateApptData['ktt_msm_status'] = 'pending';
                            }
                            if ($existing_appointment['requires_ktt_ttn_review'] == 1) {
                                $updateApptData['ktt_ttn_status'] = 'pending';
                            }
                        }
                        
                        // Handle updated_at = NOW() using prepared statement logic
                        $set_parts = [];
                        $params = [];
                        $types = "";
                        
                        foreach ($updateApptData as $col => $val) {
                            $set_parts[] = "$col = ?";
                            $params[] = $val;
                            $types .= "s";
                        }
                        ...
                        $update_sql = "UPDATE appointments SET " . implode(', ', $set_parts) . " WHERE id = ?";
                        $params[] = $appointment_id;
                        $types .= "i";

                        if ($db->query($update_sql, $params, $types)) {
