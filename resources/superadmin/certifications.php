...
$page_title = 'Certifications Master Data';
require_once dirname(__DIR__, 2) . '/bootstrap/app.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create' && hasPermission('certification.create')) {
            ...
            $res = $helper->createRecord('certifications', $data, 'cert_name', $data['cert_name']);
            if ($res['status'] === 'success') $success_msg = $res['message'];
            else $error_msg = $res['message'];
        } 
        elseif ($action === 'update' && hasPermission('certification.update')) {
            $id = (int)$_POST['id'];
            $data = [
                'cert_name'         => $_POST['cert_name'],
                'description'       => $_POST['description'],
            ...
            $res = $helper->updateRecord('certifications', $id, $data, 'cert_name', $data['cert_name']);
            if ($res['status'] === 'success') $success_msg = $res['message'];
            else $error_msg = $res['message'];
        } 
        elseif ($action === 'delete' && hasPermission('certification.delete')) {
            $id = (int)$_POST['id'];
            $res = $helper->deleteOrDeactivateRecord('certifications', $id);
...
$dataRes = $helper->getPaginatedData('certifications', $page, $limit, $search, $filters, ['cert_name', 'issuing_authority']);
...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="mb-0 fw-bold text-dark">Certifications</h2>
        </div>
        <?php if(hasPermission('certification.create')): ?>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body d-flex align-items-center">
            <h5 class="mb-0 text-muted fw-bold">Total Certifications: <span class="badge bg-primary ms-2" style="font-size: 1rem;"><?php echo $totalRecords; ?></span></h5>
        </div>
    </div>
                ...
                <?php else: ?>
                    <?php foreach($records as $row): ?>
                        <tr>
                            ...
                            <td class="text-end">
                                <?php if(hasPermission('certification.update')): ?>
                                <button class="action-btn" onclick='editData(<?php echo json_encode($row); ?>)'><i class="fas fa-edit text-primary"></i></button>
                                <?php endif; ?>
                                
                                <?php if(hasPermission('certification.delete')): ?>
                                <button class="action-btn" onclick='deleteData(<?php echo $row['id']; ?>, "<?php echo htmlspecialchars($row['cert_name'], ENT_QUOTES); ?>")'><i class="fas fa-trash-alt text-danger"></i></button>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<div class="modal fade" id="deleteModal" tabindex="-1">
    ...
    <?= csrf_field() ?>
                ...
                <div class="modal-body">
                    ...
                    <p class="text-muted small">If this record is tied to employees or requirements, it will be safely deactivated.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
